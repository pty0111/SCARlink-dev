...
def plot_scarlink_output(scarlink_out, genes, celltype, features_to_plot = None, plot_frags = False, to_save = True, plot_file = '', plot_dir = '', cmap = None, save_format='png', figsize=(17, 14), sort_gex=True, show_yticks=False, shap_cmap='Blues', pvals_cmap='Blues', cluster_order=[], plot_shap=True, bg_transparent=False):
    if isinstance(genes, str): genes = [genes]
    plot_dir_input = scarlink_out['plot_dir'] if plot_dir == '' else plot_dir
    for gene in genes:
